...
//  VSeeClinicKit
//
//  Created by Diazz on 26/10/17.
//  Copyright © 2018 Diazz. All rights reserved.
//
 ...
 * @version 1.0
 */
@objc open class FileUtil: NSObject {

    // Subdirectory name for saved files
    static let CONTENT_DIR = "content"
    
    class func getImageFileNameFromLocalId(_ localId: Int) -> String {
        return "local_image_\(localId)"
    }
    
    @objc public class func saveLocalImage(_ localId: Int, image: UIImage) -> URL? {
        return saveContentFile(getImageFileNameFromLocalId(localId), data: image.jpegData(compressionQuality: 0.5)!)
    }
    
    @objc public class func removeLocalImageFile(_ localId: Int) {
        removeFile(getImageFileNameFromLocalId(localId))
    }
    
    @objc public class func getLocalImageFile(_ localId: Int) -> Data? {
        let url = getLocalImageFileURL(localId)
        do {
    ...
    @objc public class func getLocalImageFileURL(_ localId: Int) -> URL {
        return getLocalFileURL(getImageFileNameFromLocalId(localId))
    }
